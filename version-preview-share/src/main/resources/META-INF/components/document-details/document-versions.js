...
(function()
{
   ...
   YAHOO.extend(Alfresco.DocumentVersions, Alfresco.component.Base,
   {
      ...
      getDocumentVersionMarkup: function DocumentVersions_getDocumentVersionMarkup(doc)
      {
         ...
         html += '      <a href="#" name=".onVersionClick" rel="' + versionPreviewURL + '" class="' + this.id + ' versionPreview" title="' + this.msg("label.versionPreview") + '">&nbsp;</a>'; //Versions Preview new action
         html += '   </span>';
       ...
       * @param version
       */
      onVersionClick: function DocumentVersions_onVersionClick(versionPreviewURL)
      {
    	  
    	  //Versions preview new action
    	  var win = window.open(versionPreviewURL);  ///similar to above solution
    	  win.focus();
      },
